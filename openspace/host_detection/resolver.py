...
def _try_read_claude_code_config() -> Optional[Dict[str, Any]]:
    """Read Claude Code local settings as a last-resort local runtime fallback.

    Expected shape in ``~/.claude/settings.json``:
      {
        "env": {
    ...
    Returns a litellm-compatible dict plus ``_model`` when available.
    """
    ...
    try:
        ...
        logger.warning("Failed to read Claude Code settings %s: %s", settings_path, exc)
        return None
    ...
    api_key = str(env.get("ANTHROPIC_AUTH_TOKEN", "")).strip()
    api_base = str(env.get("ANTHROPIC_BASE_URL", "")).strip()
...
def build_llm_kwargs(model: str) -> tuple[str, Dict[str, Any]]:
    ...
    Tier 4 — Claude Code local settings fallback (only when Tier 1+2+3 absent)::
    ...
    if not has_explicit_llm_override and not provider_native_env_used:
        ...
        if not host_config:
            host_config = _try_read_claude_code_config()
            if host_config:
                host_source = "claude code settings"
