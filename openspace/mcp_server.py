...
async def _get_openspace():
    ...
    async with _openspace_lock:
        ...
        disable_cloud_community = os.environ.get("OPENSPACE_DISABLE_CLOUD_COMMUNITY", "true").lower() in ("true", "1", "yes")
        enable_claude_evolution = os.environ.get("OPENSPACE_ENABLE_CLAUDE_EVOLUTION", "false").lower() in ("true", "1", "yes")
        enable_claude_deprecation = os.environ.get("OPENSPACE_ENABLE_CLAUDE_DEPRECATION", "false").lower() in ("true", "1", "yes")
        enable_skill_deprecation = os.environ.get("OPENSPACE_ENABLE_SKILL_DEPRECATION", "false").lower() in ("true", "1", "yes")
        ...
        config = OpenSpaceConfig(
            ...
            disable_cloud_community=disable_cloud_community,
            enable_claude_evolution=enable_claude_evolution,
            enable_claude_deprecation=enable_claude_deprecation,
            enable_skill_deprecation=enable_skill_deprecation,
        )
...
@mcp.tool()
async def analyze_claude_artifacts(
    write_drafts: bool = True,
) -> str:
    """Analyze `.claude` artifacts in the current workspace.

    This check is contract-driven and deterministic. It only writes drafts
    under `.claude/evolution-drafts/` when explicitly enabled.
    """
    try:
        openspace = await _get_openspace()
        manager = openspace.get_claude_artifact_manager()
        if not manager or not manager.is_enabled():
            return _json_error("No .claude workspace is enabled for the current OpenSpace instance.")

        evaluations = openspace.evaluate_claude_artifacts(write_drafts=False)
        ...
        draft_paths = manager.write_evaluation_drafts(evaluations) if write_drafts else []
        return _json_ok(
            {
                "status": "success",
                "workspace": str(manager.workspace.workspace_root if manager.workspace else ""),
                "artifact_count": len(evaluations),
                "finding_count": finding_count,
                "severity_counts": severity_counts,
                "draft_count": len(draft_paths),
                "draft_root": str(manager.workspace.evolution_drafts_dir if manager.workspace else ""),
                "artifacts": [
                    {
                        "artifact_id": evaluation.artifact.artifact_id,
                        "artifact_type": evaluation.artifact.artifact_type.value,
        ...
        logger.error(f"analyze_claude_artifacts failed: {e}", exc_info=True)
        return _json_error(e)


@mcp.tool()
async def propose_claude_evolution(
    write_drafts: bool = True,
) -> str:
    """Generate deterministic repair proposals for `.claude` artifacts.

    Proposals are derived from contract findings and stay in draft form. They
    do not directly modify formal `.claude` files.
    """
    try:
        openspace = await _get_openspace()
        manager = openspace.get_claude_artifact_manager()
        ...
        proposals = openspace.propose_claude_evolution(write_drafts=False)
        draft_paths = manager.write_proposal_drafts(proposals) if write_drafts else []
        action_count = sum(len(item.actions) for item in proposals)
        return _json_ok(
            {
                "status": "success",
                "workspace": str(manager.workspace.workspace_root if manager.workspace else ""),
                ...
                "artifacts": [
                    {
                        ...
                        "artifact_type": proposal.artifact.artifact_type.value,
                        "path": str(proposal.artifact.path),
        ...
        logger.error(f"propose_claude_evolution failed: {e}", exc_info=True)
        return _json_error(e)


@mcp.tool()
async def plan_claude_patches(
    write_drafts: bool = True,
) -> str:
    """Generate structured patch plans for `.claude` artifacts.

    Patch plans are deterministic draft instructions. They describe what to
    patch and where, but do not directly modify formal `.claude` files.
    """
    try:
        openspace = await _get_openspace()
        manager = openspace.get_claude_artifact_manager()
        ...
        plans = openspace.plan_claude_patches(write_drafts=False)
        draft_paths = manager.write_patch_plan_drafts(plans) if write_drafts else []
                ...
                "artifacts": [
                    {
                        ...
                        "artifact_type": plan.artifact.artifact_type.value,
                        ...
                        "steps": [
                            {
                                ...
                                "anchor_hint": step.anchor_hint,
                                "sample_text": step.sample_text,
                            }
        ...
        logger.error(f"plan_claude_patches failed: {e}", exc_info=True)
        return _json_error(e)


@mcp.tool()
async def draft_claude_patches(
    write_drafts: bool = True,
) -> str:
    """Generate review-ready patch drafts for `.claude` artifacts.

    Patch drafts are built from the current file contents plus structured patch
    plans. They remain draft-only and do not overwrite formal `.claude` files.
    """
    try:
        openspace = await _get_openspace()
        manager = openspace.get_claude_artifact_manager()
        ...
        drafts = openspace.draft_claude_patches(write_drafts=False)
        draft_paths = manager.write_patch_drafts(drafts) if write_drafts else []
        block_count = sum(len(item.blocks) for item in drafts)
        return _json_ok(
            {
                "status": "success",
                "workspace": str(manager.workspace.workspace_root if manager.workspace else ""),
                "draft_object_count": len(drafts),
                ...
                "artifacts": [
                    {
                        ...
                        "artifact_type": draft.artifact.artifact_type.value,
                        "path": str(draft.artifact.path),
                        "summary": draft.summary,
                        "blocks": [
                            {
                                "operation": block.step.operation,
                                "target_area": block.step.target_area,
                                "instruction": block.step.instruction,
                                "anchor_hint": block.step.anchor_hint,
                                "current_excerpt": block.current_excerpt,
                                "proposed_excerpt": block.proposed_excerpt,
                            }
        ...
        logger.error(f"draft_claude_patches failed: {e}", exc_info=True)
        return _json_error(e)
