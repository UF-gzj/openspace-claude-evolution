...
from openspace.claude_artifacts.patch_drafts import ArtifactPatchDraft
from openspace.claude_artifacts.evaluation import ArtifactEvaluation
from openspace.claude_artifacts.patch_plans import ArtifactPatchPlan
from openspace.claude_artifacts.proposals import ArtifactProposal
from openspace.lifecycle import LifecycleManager
...
class OpenSpaceConfig:
    ...
    # Claude compatibility & lifecycle governance
    disable_cloud_community: bool = True
    enable_claude_evolution: bool = False
    enable_claude_deprecation: bool = False
    enable_skill_deprecation: bool = False
...
class OpenSpace:
    def __init__(self, config: Optional[OpenSpaceConfig] = None):
        ...
        self._claude_artifact_manager: Optional[ClaudeArtifactManager] = None
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(f"[OK] LLM Client: {self.config.llm_model}")
            ...
            logger.info(f"[OK] Grounding Client: {len(backends)} backends")
            ...
            if self.config.enable_recording:
                ...
                logger.info(f"[OK] Recording enabled: {len(self._recording_manager.backends or [])} backends")
            ...
            if self.config.tool_retrieval_model:
                ...
                logger.info(f"[OK] Tool retrieval LLM: {self.config.tool_retrieval_model}")
            ...
            logger.info(f"[OK] GroundingAgent: {', '.join(backend_scope)}")
            ...
            if self._grounding_config and self._grounding_config.skills.enabled:
                ...
                if self._skill_registry:
                    ...
                    logger.info(f"[OK] Skills: {len(skills)} discovered")
            ...
            if self.config.enable_recording and self._skill_registry:
                try:
                    ...
                    logger.info("[OK] Execution analysis enabled")
                    ...
                    logger.info(
                        f"[OK] Skill evolution enabled "
                        f"(concurrent={self.config.evolution_max_concurrent})"
                    )
                except Exception as e:
                    logger.warning(f"Execution analyzer init failed (non-fatal): {e}")

            if self.config.enable_claude_evolution or self.config.enable_claude_deprecation:
                self._claude_artifact_manager = ClaudeArtifactManager(self.config.workspace_dir)
                if self._claude_artifact_manager.is_enabled():
                    draft_dirs = self._claude_artifact_manager.ensure_draft_dirs()
                    artifact_count = len(self._claude_artifact_manager.discover_artifacts())
                    workspace = self._claude_artifact_manager.workspace
                    logger.info(
                        "[OK] Claude artifact compatibility enabled: %s artifact(s), drafts at %s",
                        artifact_count,
                        workspace.evolution_drafts_dir if workspace else "",
                    )
                    logger.debug("Claude draft dirs: %s", draft_dirs)
                else:
                    logger.info(
                        "Claude artifact compatibility requested, but no .claude workspace was detected under %s",
                        self.config.workspace_dir,
                    )

            if self.config.enable_claude_deprecation or self.config.enable_skill_deprecation:
                self._lifecycle_manager = LifecycleManager(
                    self.config.workspace_dir,
                    claude_manager=self._claude_artifact_manager,
                )
                logger.info("[OK] Lifecycle governance enabled: %s", self._lifecycle_manager.store_path)

                if self.config.enable_skill_deprecation and self._skill_registry:
                    synced_skills = self._lifecycle_manager.sync_native_skills(
                        self._skill_registry.list_skills()
                    )
                    logger.info("  Synced native skills into lifecycle store: %s", synced_skills)

                if self.config.enable_claude_deprecation and self._claude_artifact_manager:
                    synced_artifacts = self._lifecycle_manager.sync_claude_artifacts(
                        self._claude_artifact_manager.discover_artifacts()
                    )
                    logger.info("  Synced .claude artifacts into lifecycle store: %s", synced_artifacts)
    ...
    def get_claude_artifact_manager(self) -> Optional[ClaudeArtifactManager]:
        return self._claude_artifact_manager

    def get_lifecycle_manager(self) -> Optional[LifecycleManager]:
        return self._lifecycle_manager

    def evaluate_claude_artifacts(self, *, write_drafts: bool = False) -> List[ArtifactEvaluation]:
        if not self._claude_artifact_manager or not self._claude_artifact_manager.is_enabled():
            raise RuntimeError("Claude artifact compatibility is not enabled for the current workspace")
        evaluations = self._claude_artifact_manager.evaluate_artifacts()
        if write_drafts:
            self._claude_artifact_manager.write_evaluation_drafts(evaluations)
        return evaluations

    def propose_claude_evolution(
        self,
        *,
        ...
        if not self._claude_artifact_manager or not self._claude_artifact_manager.is_enabled():
            raise RuntimeError("Claude artifact compatibility is not enabled for the current workspace")
        evaluations = self._claude_artifact_manager.evaluate_artifacts()
        proposals = self._claude_artifact_manager.build_proposals(evaluations)
        if write_drafts:
            self._claude_artifact_manager.write_proposal_drafts(proposals)
        return proposals

    def plan_claude_patches(
        self,
        *,
        write_drafts: bool = False,
        ...
        if not self._claude_artifact_manager or not self._claude_artifact_manager.is_enabled():
            raise RuntimeError("Claude artifact compatibility is not enabled for the current workspace")
        proposals = self._claude_artifact_manager.build_proposals(
            self._claude_artifact_manager.evaluate_artifacts()
        )
        plans = self._claude_artifact_manager.build_patch_plans(proposals)
        if write_drafts:
            self._claude_artifact_manager.write_patch_plan_drafts(plans)
        return plans

    def draft_claude_patches(
        self,
        *,
        write_drafts: bool = False,
        ...
        if not self._claude_artifact_manager or not self._claude_artifact_manager.is_enabled():
            raise RuntimeError("Claude artifact compatibility is not enabled for the current workspace")
        plans = self._claude_artifact_manager.build_patch_plans(
            self._claude_artifact_manager.build_proposals(
                self._claude_artifact_manager.evaluate_artifacts()
            )
        )
        drafts = self._claude_artifact_manager.build_patch_drafts(plans)
        if write_drafts:
            self._claude_artifact_manager.write_patch_drafts(drafts)
        return drafts
