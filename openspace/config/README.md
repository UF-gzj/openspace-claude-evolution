...
| **Tier 4** | Claude Code local settings | `~/.claude/settings.json` |
...
| `OPENSPACE_DISABLE_CLOUD_COMMUNITY` | Disable cloud community features by default | `true` |
...
| `OPENSPACE_ENABLE_CLAUDE_EVOLUTION` | Enable `.claude` artifact compatibility and draft generation | `false` |
| `OPENSPACE_ENABLE_CLAUDE_DEPRECATION` | Enable `.claude` artifact deprecation lifecycle | `false` |
| `OPENSPACE_ENABLE_SKILL_DEPRECATION` | Enable native skill soft-deprecation lifecycle | `false` |
