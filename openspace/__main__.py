...
async def refresh_mcp_cache(config_path: Optional[str] = None):
    ...
    print()
    print(f"{'='*50}")
    print(f"[OK] Collected {total_tools} tools from {success_count + skipped_count}/{total} servers")
    if skipped_count > 0:
        print(f"  (skipped {skipped_count} cached, processed {success_count} new)")
    print(f"[OK] Cache saved to: {cache.cache_path}")
...
def _load_config(args) -> OpenSpaceConfig:
    ...
    cli_overrides['disable_cloud_community'] = disable_cloud_community
    cli_overrides['enable_claude_evolution'] = enable_claude_evolution
    cli_overrides['enable_claude_deprecation'] = enable_claude_deprecation
    cli_overrides['enable_skill_deprecation'] = enable_skill_deprecation
    ...
    try:
        ...
        if args.config:
            ...
            print(f"[OK] Loaded from config file: {args.config}")
        else:
            config = OpenSpaceConfig(**cli_overrides)
            print("[OK] Using default configuration")
        
        if args.model:
            print(f"[OK] CLI overrides: llm_model")
