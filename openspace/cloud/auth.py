...
def is_cloud_community_disabled() -> bool:
    return os.environ.get("OPENSPACE_DISABLE_CLOUD_COMMUNITY", "true").strip().lower() in ("true", "1", "yes")
...
def get_openspace_auth() -> tuple[Dict[str, str], str]:
    ...
    if is_cloud_community_disabled():
        logger.info("OpenSpace cloud community is disabled by OPENSPACE_DISABLE_CLOUD_COMMUNITY")
...
def get_auth_headers_or_exit() -> Dict[str, str]:
    ...
    if is_cloud_community_disabled():
        print(
            "ERROR: Cloud community is disabled in this fork.\n"
            "  This project defaults to local and private evolution only.\n"
            "  If you really need cloud access, explicitly set OPENSPACE_DISABLE_CLOUD_COMMUNITY=false.",
            file=sys.stderr,
        )
        sys.exit(1)
